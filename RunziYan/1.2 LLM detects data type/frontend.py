...
with col1:
    ...
    if uploaded_file is not None:
            ...
            if file_id not in st.session_state.processed_files:
                with st.spinner(" GeoGPT analysiert die Datei und strukturiert den Layer..."):
                    try:
                        # 1. 基础解析（利用后台现有的基础解析工厂转换成 Python 字典/列表）
                        parsed_data = parse_uploaded_file(uploaded_file)
                        raw_data = parsed_data["data"]

                        # 2. 智能化数据切片（只抽取前几条数据作为 Sample，避免大文件撑爆大模型 Token 限制）
                        if isinstance(raw_data, list):
                            # 如果是 CSV/JSON 点列表，取前 3 条记录
                            sample_str = json.dumps(raw_data[:3], ensure_ascii=False)
                        elif isinstance(raw_data, dict) and "features" in raw_data:
                            # 如果是标准 GeoJSON，切出前 2 个 Feature 供模型观察其 Properties 属性
                        ...
                        # 4. 组装契合你们原有架构的统一图层对象 (Layer Object)
                        current_layer = {
                            "name": recommended_name,          # 大模型赋予的、人类友好的优雅名字
                            "type": detected_type,             # 大模型动态识别的类型
                            "data": raw_data,
                            "style": copy.deepcopy(st.session_state.map_style),
                            "data_fields": detected_fields     # 大模型帮你淘洗出来的核心指标字段
                        }

                        # 5. 更新系统的全局状态和缓存，无缝对接后续的地图生成逻辑
                        ...
                        st.success(f"🎉 Erfolgreich! Layer hinzugefügt als: **{recommended_name}** ({detected_type})")

                    except Exception as e:
                        st.error(f"Fehler bei der intelligenten Analyse: {e}")

...
def parse_uploaded_file(uploaded_file):
    ...
    if filename.endswith(".geojson"): return {"type": "geojson", "data": json.load(uploaded_file)}
...
def parse_json_points(data):
    ...
    for i, item in enumerate(data):
        ...
        if lat is not None and lon is not None:
            points.append({"name": item.get("name", f"Point {i+1}"), "lat": float(lat), "lon": float(lon)})
    return points

def parse_csv_points(df):
    points = []
    for i, row in df.iterrows():
        points.append({"name": row["name"] if "name" in df.columns else f"Point {i+1}", "lat": float(row["lat"]), "lon": float(row["lon"])})
    return points
# %%
